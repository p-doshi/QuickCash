...
public class EmployerPaymentUIAutomatorTest {
    ...
    private static final String ALLOW_CAM = "While using the app";
    private static final String CARD_BOX = "1234 5678";
    private static final String ACCEPT_CARD = "5317805091683572";
    private static final String DECLINED_CARD = "5317805091683572";
    ...
    @Test
    public void checkIfMovedToDashboard() throws UiObjectNotFoundException, InterruptedException {
        ...
        UiObject allowCameraButton = device.findObject(new UiSelector().textContains(ALLOW_CAM));
        if (allowCameraButton.exists()) {
            allowCameraButton.clickAndWaitForNewWindow();
        }
    ...
    @Test
    public void declinedCard() throws UiObjectNotFoundException, InterruptedException {
        UiObject paymentConfirmButton = device.findObject(new UiSelector().textContains(CONFIRM));
        paymentConfirmButton.clickAndWaitForNewWindow();
        UiObject payCardButton = device.findObject(new UiSelector().textContains(PAY_CARD));
        payCardButton.clickAndWaitForNewWindow();
